...
public class GraphicDecoration implements Decoration {
   ...
   *
   * <p>You should not need to call this method, because ValidatorFX should detect the need to
   * update graphic validations. If you do need to call this method, consider filing an issue at
   * https://github.com/effad/ValidatorFX/issues
   *
  ...
  public static void updateDecorations(Node node) {
    ...
    if (decorationPane != null) {
      decorationPane
          .requestLayout(); // will trigger the layoutListener for all graphic decorations in scene
    }
  }
  ...
  private void layoutGraphic() {
    ...
    switch (pos.getVpos()) {
      ...
      case BASELINE:
        y +=
            decoratedNode.getBaselineOffset()
                - decorationNode.getBaselineOffset()
