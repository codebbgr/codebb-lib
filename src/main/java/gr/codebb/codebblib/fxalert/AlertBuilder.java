...
/** Contains API that simplifies creating JavaFX {@code javafx.scene.control.Alert}s */
public class AlertBuilder implements IDialogBuilder<AlertBuilder> {

  /**
   * Default constructor, creates a default AlertBuilder with type {@code
  ...
  /**
   * Creates an AlertBuilder suitable for the passed {@code javafx.scene.control.Alert.AlertType}
   *
  ...
  /**
   * Creates an AlertBuilder suitable for creating Alerts that display {@code Exception} stack
   * traces.
   *
  ...
  public AlertBuilder withTitleBarIcon(Image image) {
    ((Stage) this.alert.getDialogPane().getScene().getWindow()).getIcons().add(image);
    return this;
  }
